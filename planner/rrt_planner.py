...
def planner(env,start,goal,numofDOFs):
	#print(env[0])
	map = np.loadtxt(env)
	x_size = map.shape[0]
	y_size = map.shape[1]
	...
	for k in range(max_expand_times):
		...
		if (k % 2 == 0):
			...
			ExtendStatus result = extend(q_rand, q_new, node_list_backward, step_size, interpolate_times, numofDOFs, map, x_size, y_size)
